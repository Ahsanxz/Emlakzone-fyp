    ...
    func DataLoader()
        {
            ...
            let networking = Networking(baseURL: "http://server.emlakzone.com/public/api/v1/search")
                networking.get("/properties?purpose=sale&size=10&offset=" + "\(offsetSize)") { result in
                   switch result {
            ...
            if propeties["agency"]["guid"].stringValue == "https://server.emlakzone.com/public/img/default-thumb.png"{
                        ...
                        let agencyTitl = propeties["agency"]["title"]["en"].stringValue
                        self.agencyTitle_Arr.append(agencyTitl)
